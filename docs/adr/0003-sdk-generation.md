...
That turned out to be true. Zig landed second and needed nothing added to the
description: the struct layouts, the `optional` flag, the `sized_by` link and
the ownership notes were all already there, and the backend is one more module
...
finalizer and a cache of live handles, in the Python crate `arena.rs`.
...
- The description was rich enough for a second target without being changed,
