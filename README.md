...
| [C#](sdk/csharp) | `sdk/csharp` | `libotio`, loaded at run time |
| [Objective-C](sdk/objc) | `sdk/objc` | `libotio`, on Apple's runtime and GNUstep's |
...
cp target/release/libotio.a sdk/objc/lib/  && (cd sdk/objc  && make check)
```

C# loads the shared library at run time instead of linking the static one:

```sh
cp target/release/libotio.so sdk/csharp/lib/ && \
  (cd sdk/csharp && dotnet run --project tests)   # libotio.dylib on macOS
```
...
cargo run -p otio-sdk-gen -- ts        # go | swift | ts | zig | cpp | csharp | objc
