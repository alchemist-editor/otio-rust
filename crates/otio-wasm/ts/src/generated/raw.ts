 ...
 *
 * `shallow` stops at the composition's own children rather than descending into
 * nested ones. Answers `undefined` when nothing covers it.
 */
 ...
 * none.
 *
 * With `OTIO_NEIGHBOR_GAP_AROUND_TRANSITIONS`, a transition at the head or tail
 * of a track gets a gap materialized for its overhang, which is why this takes
 * a document it may edit.
 */
 ...
 *
 * Handles into the original name the same objects in the copy, because the copy
 * keeps the arena's layout.
 */
 ...
 *
 * Answers `undefined` for a document that has none, which is what a freshly
 * created one is.
 */
...
/**
 * Drops an item into a gap on a track, fitting it as the reference point says.
 */
 ...
 *
 * `out_name` may be null if the name is not wanted. Answers `undefined` for an
 * untinted item.
 */
 ...
 *
 * Removing a key of a dictionary takes the key with it; removing an element of
 * an array shortens the array.
 */
 ...
 *
 * The object stays where it is and the metadata names it. Nothing checks that
 * the handle is live, because a document being built may not hold the object
 * yet.
 */
 ...
 *
 * Tracks, stacks, bare compositions and serializable collections hold children;
 * anything else reports an `OtioError` whose status is `"coreError"`.
 */
 ...
 * report this. Compare against it with `undefined`.
 */
 ...
 *
 * Only a transition does, which is why one does not advance the playhead when a
 * track is laid out.
 */
...
/**
 * Returns the name of a status code, such as `"an `OtioError` whose status is
 * `"ok"`"`.
 *
 * The string is static and needs no freeing.
 */
 ...
 * `Node#remove` call. That is the same bargain `Composition#detachChild` makes,
 * and leaving its parent pointing at the timeline instead would mean an object
 * claiming a parent that has disowned it. A displaced stack that some other
 * timeline has since taken as its own keeps that timeline as its parent, since
 * this one is not the timeline disowning it.
 */
