...
use aaf::write::{
    AafWriter, Clock, IdSource, Rational, SequentialIds, SteppingClock, Timestamp, WriteOptions,
    WriteValue,
};
...
fn writer_for(name: &'static str) -> (AafWriter, Replay, Vec<u8>) {
    ...
    let sidecar =
        std::fs::read_to_string(dir.join(format!("{name}.calls.tsv"))).expect("sidecar exists");
    ...
    for line in sidecar.lines().filter(|l| !l.starts_with('#')) {
        ...
        if kind == "sector_size" {
            sector_size = value.parse().expect("a sector size");
        } else {
            calls.push_back((kind.to_owned(), value.to_owned()));
        }
    }
    let replay = Replay {
        name,
...
fn locate(bytes: &[u8], offset: usize) -> String {
...
fn assert_identical(name: &str, ours: &[u8], theirs: &[u8]) {
    if ours == theirs {
        return;
    }
    let first = ours
