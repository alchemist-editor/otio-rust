...
#
#     python3 gen_metadict.py ~/src/pyaaf2
#
    ...
    with aaf2.open(path, 'r') as f:
        ...
        for c in md['ClassDefinitions'].values():
            ...
            rows.append('C\t%s\t%s\t%s\t%s\t%s' % (a(c, P['AUID']), s(c, P['NAME']),
                                                   ...
                                                   '|'.join(props)))
        for t in md['TypeDefinitions'].values():
...
run(os.path.join(FIXTURES, 'empty.aaf'), os.path.join(D, 'empty.metadict.tsv'))
run(os.path.join(FIXTURES, 'sector_size_512.aaf'), os.path.join(D, 'sector_size_512.metadict.tsv'))
