//! Python bindings for `opentime`.
//!
//! This is the `opentimelineio._opentime` extension module, and it is a
...
#[derive(Clone, Copy)]
pub struct PyRationalTime(RationalTime);
...
#[derive(Clone, Copy)]
pub struct PyTimeRange(TimeRange);
...
#[derive(Clone, Copy)]
pub struct PyTimeTransform(TimeTransform);
...
/// The `opentimelineio._opentime` extension module.
#[pymodule]
fn _opentime(module: &Bound<'_, PyModule>) -> PyResult<()> {
