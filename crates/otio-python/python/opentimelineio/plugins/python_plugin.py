...
def _already_imported(path):
    """Return the module already imported from the file at path, if any."""
    ...
    for mod in list(sys.modules.values()):
        ...
        if (
            mod_file
            and os.path.normcase(os.path.realpath(mod_file)) == target
        ):
            return mod
    return None
...
class PythonPlugin(core.SerializableObject):
    ...
    def _imported_module(self, namespace):
        ...
        try:
            ...
            # Not in upstream: a module file that is already imported is used
            # as it is, rather than executed a second time as a new module.
            # This package's own AAF adapter is ``advanced_authoring_format``
            # under the plugin name ``AAF``, and a second copy of it would
            # raise an ``AAFAdapterError`` that is not the one callers catch.
            mod = _already_imported(self.module_abs_path())
            if mod is not None:
                return mod
