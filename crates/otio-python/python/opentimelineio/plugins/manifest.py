...
def load_manifest():
    ...
    # The file formats this package implements in Rust. Upstream ships each
    # of them as a plugin package of its own, found through an entry point and
    # declared by a plugin_manifest.json at the top of that package. They are
    # part of this package here, so its own plugin_manifest.json declares them
    # the same way, and it is read with the builtin manifest -- after it, and
    # regardless of OTIO_DISABLE_ENTRYPOINTS_PLUGINS -- so that everything
    # found before still takes precedence over them.
    formats_manifest_path = (
        resources.files("opentimelineio") / "plugin_manifest.json"
    ).as_posix()

    if os.path.abspath(formats_manifest_path) not in result.source_files:
        plugin_manifest = manifest_from_file(formats_manifest_path)
        result.extend(plugin_manifest)
