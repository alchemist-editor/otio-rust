...
"""The time model: times, ranges and transforms.

Carried over from upstream's ``opentimelineio/opentime.py`` unchanged, because
it is part of the API a caller sees: the module-level functions here are what
most code actually calls, and they are thin wrappers over methods on the
classes the ``_opentime`` extension module provides.
"""
...
def to_time_string(rt):
    ...
    ``hh:mm:ss`` where ``ss`` is an integer or a decimal number.
    """
