...
int main(void) {
    ...
    OtioNode media;
    otio_external_reference_new(document, "A001", "file:///A001.mov", &media);
    ...
    otio_media_reference_set_available_range(document, media, available);
    ...
    OtioNode clip;
    otio_clip_new(document, "shot", &clip);
    otio_clip_set_media_reference(document, clip, "DEFAULT_MEDIA", media);
    ...
    otio_item_set_source_range(document, clip, source);

    /* A second of black in front of it, so the clip does not start the
     * track. */
    OtioNode head;
    otio_gap_new(document, NULL, &head);
    ...
    otio_item_set_source_range(document, head, black);

    OtioNode track;
    otio_track_new(document, "V1", "Video", &track);
    otio_composition_append_child(document, track, head);
    ...
    OtioTimeRange range;
    otio_item_available_range(document, clip, &range);
    print_range("available:", range);
    otio_item_trimmed_range(document, clip, &range);
    print_range("trimmed:  ", range);
    otio_item_visible_range(document, clip, &range);
    print_range("visible:  ", range);
    otio_item_range_in_parent(document, clip, &range);
